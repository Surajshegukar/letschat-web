...
import { ChevronRight, FileText, Star, Users, LucideIcon } from "lucide-react";
...
const defaultItems: SharedAssetItem[] = [
...
export function DetailsSharedAssets({ items = defaultItems, onOpenMedia }: DetailsSharedAssetsProps) {
  return (
    <div className="space-y-1.5">
      {items.map((item) => (
        <button
          key={item.label}
          onClick={item.label === "Media, Links & Files" ? onOpenMedia : undefined}
        ...
        >
          ...
          <div className="flex items-center gap-1.5">
            <span className="text-xs font-bold text-zinc-400 dark:text-zinc-100 bg-zinc-100 dark:bg-zinc-900 px-2 py-0.5 rounded-full">
              {item.count}
            </span>
